...
class RCModel(object):
    ...
    def get_loss_v2(self):
        return self._compute_loss_v2()
    def get_loss(self):
        return self._compute_loss()

    def get_prediction(self):
        return self.start_probs, self.end_probs
    ...
    def _setup_input(self, input):
        self.p = input['passage_tokens'] # tf.int32, [None, None]
        self.q = input['question_tokens'] # tf.int32, [None, None]
        self.p_length = input['passage_length'] # tf.int32, [None]
        self.q_length = input['question_length'] # tf.int32, [None]
        self.start_label = input['start_label'] # tf.int32, [None]
        self.end_label = input['end_label'] # tf.int32, [None]
        self.start_label_probs = input['start_label_probs'] # tf.float32 [None, None]
        self.end_label_probs = input['end_label_probs'] # tf.float32 [None, None]
        self.dropout_keep_prob = input['dropout_keep_prob'] # tf.float32, []
#        self.p = tf.Print(self.p, [tf.shape(self.p)], message = 'passage batch per gpu:')
        # add char-level embeding
        if hasattr(self, 'char_vocab'):
            # batch x pragraph_len x token_len
            self.p_char = input['passage_chars'] #[None, None, None]
            self.q_char = input['question_chars'] #(tf.int32, [None, None, None])
            self.p_char_length = input['passage_chars_length'] #(tf.int32, [None, None])
            self.q_char_length = input['question_chars_length'] #(tf.placeholder(tf.int32, [None, None])
